...
@dataclass
class RTCDemoConfig:
    """Configuration for RTC demo with dual model support.
    
    注意：此配置类不再使用单一的 policy 字段，而是在 demo_cli 中分别加载两个模型。
    policy 字段仅作为占位符，用于保持与解析器的兼容性。
    """

    # RTC configuration (两个模型共享相同的RTC配置)
    rtc: RTCConfig = field(
...
def get_actions(
    ...
    try:
        ...
        # 使用model_bundle中的preprocessor和postprocessor
...
def load_model_bundle(
    ...
    """
    logger.info(f"[LOAD] Loading model '{model_name}' from {model_path}")
    ...
    logger.info(f"[LOAD] Model '{model_name}' loaded successfully")
...
def select_model(models: dict) -> str:
    """
    显示模型选择菜单并返回用户选择的模型key
    
    Args:
        models: 模型字典 {key: ModelBundle}
    ...
    for key, bundle in models.items():
        print(f"  [{key}] {bundle.name}")
        print(f"      路径: {bundle.pretrained_path}")
    print("="*60)
    
    while True:
        choice = input("请选择模型 (1=窄箱子, 2=宽箱子): ").strip()
        if choice in models:
            print(f"✓ 已选择模型: {models[choice].name}")
            return choice
        else:
            print(f"⚠ 无效选择 '{choice}'，请输入 1 或 2")
...
def demo_cli(cfg: RTCDemoConfig):
    ...
    # ========== 加载双模型 ==========
    logger.info("[MAIN] Loading both models (窄箱子 and 宽箱子)...")
    
    model_narrow = load_model_bundle(
        model_name="窄箱子模型",
        model_path=MODEL_NARROW_PATH,
        cfg=cfg,
        device=cfg.device
    )
    
    model_wide = load_model_bundle(
        model_name="宽箱子模型", 
        model_path=MODEL_WIDE_PATH,
        cfg=cfg,
        device=cfg.device
    )
    ...
    logger.info("[MAIN] 窄箱子模型和宽箱子模型加载完成!")
    ...
    while True:
        ...
        input(f"按回车开始使用 {selected_model.name} 推理...")
        ...
        logger.info(f"[MAIN] ActionQueue created, using model: {selected_model.name}")

        # 启动线程 - 传入选中的model_bundle
        get_actions_thread = Thread(
            target=get_actions, 
            args=(selected_model, env, action_queue, shutdown_event, cfg, episode_state), 
            daemon=True, 
            name="GetActions"
        )
        actor_thread = Thread(
        ...
        logger.info(f"[MAIN] Threads started with {selected_model.name}. Running demo...")
        print(f"\n💡 提示: 输入 'q' 停止当前推理并返回模型选择菜单")
        ...
        actor_thread.join()
        logger.info("[MAIN] Demo finished. Returning to model selection...")
