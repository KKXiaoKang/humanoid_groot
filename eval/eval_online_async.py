...
AGGREGATE_FUNCTIONS = {
    "weighted_average": lambda old, new: 0.2 * old + 0.8 * new,
...
class GrootAsyncClient:
    ...
    def send_observation(self, obs_data: dict, resampled_action_queue: deque) -> bool:
        """发送观测到服务器"""
    ...
    def run(self):
        ...
        try:
            ...
            while not self.shutdown_event.is_set():
                ...
                # 2. 发送观测
                obs_sent = self.send_observation(obs_data, resampled_action_queue)
                if obs_sent:
                ...
                if (len(resampled_action_queue) < 60):
                    # 从服务器动作队列获取新动作块
                    action_chunk = []
                    with self.action_queue_lock:
