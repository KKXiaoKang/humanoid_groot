...
from kuavo_humanoid_sdk.kuavo.core.ros.param import EndEffectorType, make_robot_param
...
from kuavo_msgs.msg import twoArmHandPoseCmd, ikSolveParam, armHandPose, armCollisionCheckInfo
from kuavo_msgs.srv import twoArmHandPoseCmdSrv, fkSrv, twoArmHandPoseCmdFreeSrv
from std_srvs.srv import SetBool, SetBoolRequest
from std_msgs.msg import Float64MultiArray
from kuavo_msgs.srv import lbLegControlSrv
...
class ControlRobotArm:
    ...
    def srv_set_arm_quick_mode(self, enable: bool)->bool:
        try:
    ...
    def srv_set_direct_to_wbc(self)->bool:
        try:
            rospy.wait_for_service('/enable_wbc_arm_trajectory_control', timeout=5.0)
            cli = rospy.ServiceProxy('/enable_wbc_arm_trajectory_control', changeArmCtrlMode)
            req = changeArmCtrlModeRequest()
            req.control_mode = 1
            res = cli(req)
            if res.result:
                rospy.loginfo(f"Successfully set direct to wbc")
                return True
            else:
                rospy.logwarn(f"Failed to set direct to wbc")
                return False
        except rospy.ServiceException as e:
            SDKLogger.error(f"Service call failed: {e}")
            return False
...
class KuavoRobotControl:
    ...
    def __init__(self):
        if not hasattr(self, '_initialized'):
            ...
            # 根据机器人版本判断是否需要初始化轮臂控制
            # robot_version >= 60 为轮臂模型，否则为双足模型
            try:
                kuavo_ros_param = make_robot_param()
                robot_version = kuavo_ros_param.get('robot_version')
                if robot_version is not None and int(robot_version) >= 60:
                    # 轮臂模型，初始化轮臂控制
                    self.kuavo_wheel_arm_control = WheelArmROSControl()
                else:
                    # 双足模型，不初始化轮臂控制
                    self.kuavo_wheel_arm_control = None
            except Exception as e:
                # 如果获取参数失败，默认不初始化轮臂控制
                SDKLogger.warning(f"[KuavoRobotControl] 无法获取机器人版本信息，跳过轮臂控制初始化: {e}")
                self.kuavo_wheel_arm_control = None
    ...
    def set_arm_quick_mode(self, enable: bool):
        """
            Set arm quick mode
            Arguments:
                - enable: enable arm quick mode
        """
        return self.kuavo_arm_control.srv_set_arm_quick_mode(enable)

    def set_direct_to_wbc(self)->bool:
        """
            Set direct to wbc
        """
        return self.kuavo_arm_control.srv_set_direct_to_wbc()
