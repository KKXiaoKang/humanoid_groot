...
class GrootAsyncClient:
            ...
            try:
                ...
                with self.action_queue_lock:
                    for action in timed_actions:
                        ...
                        self.action_queue.put((action.get_timestep(), action_np))
    ...
    def send_observation(self, obs_data: dict) -> bool:
        """发送观测到服务器"""
        ...
        if not (self.must_go.is_set() and queue_empty):
            return False
    ...
    def run(self):
        ...
        try:
            ...
            while not self.shutdown_event.is_set():
                ...
                # 2. 发送观测（如果需要）
                obs_sent = self.send_observation(obs_data)
                if obs_sent:
