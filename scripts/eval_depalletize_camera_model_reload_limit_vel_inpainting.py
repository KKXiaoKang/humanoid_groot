...
def run_inference_loop(policy, preprocessor, env, dataset_stats, task_description, device, 
                       ...
                       use_inpainting=False, inpainting_source="previous_pred", inpainting_length=1, guide_strength=0.5):
    """
    ...
    Args:
        ...
        inpainting_source: inpainting 的来源，可选值：
            - "previous_pred": 使用上一次执行 chunk 的最后 N 个时间步（跳过后的 chunk）
            - "ground_truth": 使用 ground truth action 的前 N 个时间步（此模式下不支持，因为实时环境没有 GT）
            - "none": 不使用 inpainting
    ...
    if use_inpainting:
        ...
        if skip_chunk_ratio > 0.0:
            print(f"   - Note: Will use last {inpainting_length} timesteps of executed chunk (after skip_chunk_ratio)")
    else:
    ...
    # Inpainting 相关变量：保存上一次执行 chunk 的归一化版本（跳过后的）
    last_normalized_executed_chunk: torch.Tensor | None = None
    ...
    if sync_mode:
        while True:
            try:
                ...
                guided_action = None
                if use_inpainting and inpainting_source == "previous_pred" and last_normalized_executed_chunk is not None:
                    # 使用上一次执行 chunk 的**最后** N 个时间步（跳过后的 chunk）
                    T_g = min(inpainting_length, last_normalized_executed_chunk.shape[1])
                    guided_action = last_normalized_executed_chunk[:, -T_g:, :].to(device)
                    if step_counter > 0:
                        rospy.loginfo(f"🎨 Using previous executed chunk (last {T_g} timesteps) for inpainting: shape {guided_action.shape}")
                ...
                # 跳过chunk的百分之多少（前面或后面）
                # 注意：需要同时处理归一化的 pred_actions 和反归一化的 action_chunk
                skip_steps = 0
                if skip_chunk_ratio > 0.0:
                    ...
                    if skip_steps >= chunk_size:
                        ...
                        if skip_chunk_from_end:
                            ...
                            # 对于归一化的 pred_actions，也跳过后面
                            if use_inpainting and inpainting_source == "previous_pred":
                                pred_actions = pred_actions[:, :-skip_steps, :]
                        else:
                            # 跳过前面的步骤
                            action_chunk = action_chunk[skip_steps:].copy()
                            rospy.loginfo(f"⏭️  Skipped first {skip_steps}/{chunk_size} steps ({skip_chunk_ratio*100:.1f}%)")
                            # 对于归一化的 pred_actions，也跳过前面
                            if use_inpainting and inpainting_source == "previous_pred":
                                pred_actions = pred_actions[:, skip_steps:, :]
                ...
                # 保存归一化的执行 chunk（用于下一次 inpainting）
                if use_inpainting and inpainting_source == "previous_pred":
                    # pred_actions 已经是归一化的，且已经根据 skip_chunk_ratio 处理过
                    # 保存为 (1, T_executed, D) 格式
                    last_normalized_executed_chunk = pred_actions.detach().cpu()
                    if step_counter % 10 == 0:  # 每 10 次打印一次，避免输出过多
                        rospy.loginfo(f"🎨 Saved normalized executed chunk for next inpainting: shape {last_normalized_executed_chunk.shape}, "
                                    f"will use last {min(inpainting_length, last_normalized_executed_chunk.shape[1])} timesteps")
    ...
    while True:
        try:
            ...
            if not resampled_action_queue:
                ...
                # 保存归一化的执行 chunk（用于下一次 inpainting）
                # 注意：这里保存的是跳过后的 chunk，即实际执行的 chunk
                if use_inpainting and inpainting_source == "previous_pred":
                    # pred_actions 已经是归一化的，且已经根据 skip_chunk_ratio 处理过
                    # 保存为 (1, T_executed, D) 格式
                    last_normalized_executed_chunk = pred_actions.detach().cpu()
...
def eval(ckpt_path, model_type, control_arm=True, control_claw=True, action_chunk_size=50, lerobot_dataset_path=None, enable_gui=False, rotate_head_camera=False, state_zero=False, task_description=None, skip_chunk_ratio=0.0, skip_chunk_from_end=False, model_action_dt=None, sync_mode=False, max_joint_velocity=None, use_inpainting=False, inpainting_source="previous_pred", inpainting_length=1, guide_strength=0.5):
    """
    ...
    while True:
        try:
            ...
            normal_exit = run_inference_loop(
                ...
                inpainting_source=inpainting_source,
                inpainting_length=inpainting_length,
                guide_strength=guide_strength
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--inpainting-source', type=str, default='previous_pred',
                        choices=['previous_pred', 'none'],
                        help='Source for inpainting: previous_pred (use previous executed chunk), or none (disabled). '
                             'Note: Will use last N timesteps of executed chunk (after skip_chunk_ratio)')
    ...
    if args.use_inpainting:
        print(f"🎨 INPAINTING MODE: Enabled")
        print(f"   - Source: {args.inpainting_source}")
        print(f"   - Length: {args.inpainting_length} timestep(s)")
        print(f"   - Guide strength: {args.guide_strength} ({'hard' if args.guide_strength >= 0.999 else 'soft'} inpainting)")
        if args.skip_chunk_ratio > 0.0:
            print(f"   - Note: Will use last {args.inpainting_length} timesteps of executed chunk (after skip_chunk_ratio)")
    else:
        print(f"🎨 INPAINTING MODE: Disabled")
    print("="*80 + "\n")
