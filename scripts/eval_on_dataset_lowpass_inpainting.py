...
def eval_on_dataset(ckpt_path,
                    ...
                    inpainting_source: str = "previous_pred",
                    inpainting_length: int = 1,
    ...
    Args:
        ...
        use_inpainting: 是否使用 inpainting（使用 guided_action 前缀）
        inpainting_source: inpainting 的来源，可选值：
            - "previous_pred": 使用上一次预测的 action chunk 的前 N 个时间步
            - "ground_truth": 使用 ground truth action 的前 N 个时间步
            - "none": 不使用 inpainting
        inpainting_length: inpainting 的长度（时间步数），默认 1
        guide_strength: inpainting 的强度（0.0-1.0），>= 0.999 使用 hard inpainting，< 0.999 使用 soft inpainting
    """
    # ----------- 一些参数 ----------------
    ...
    # 辅助函数：归一化动作（用于 inpainting）
    def normalize_actions(actions, action_dim, dataset_stats):
        """
        归一化动作（与 processor 中的逻辑一致）
        
        Args:
            actions: 输入动作，可以是 numpy array 或 torch tensor，形状为 (B, T, D) 或 (T, D) 或 (D,)
            action_dim: 动作维度
            dataset_stats: 数据集统计信息
            
        Returns:
            归一化后的动作，形状为 (B, T, D)，其中 B=1
        """
        if dataset_stats and 'action' in dataset_stats:
            ...
            if 'min' in action_stats and 'max' in action_stats:
                ...
                action_min = action_min[:action_dim]  # (D,)
                action_max = action_max[:action_dim]  # (D,)
                
                # 归一化公式：y = 2 * (x - min) / (max - min) - 1
                denom = action_max - action_min  # (D,)
                mask = denom != 0  # (D,)
                safe_denom = torch.where(mask, denom, torch.ones_like(denom))  # (D,)
                
                # 转换输入为 tensor 并确保形状正确
                if isinstance(actions, np.ndarray):
                    actions_tensor = torch.as_tensor(actions, dtype=torch.float32)
                else:
                    actions_tensor = actions if isinstance(actions, torch.Tensor) else torch.as_tensor(actions, dtype=torch.float32)
                ...
                # 确保最后一个维度匹配 action_dim
                if actions_tensor.shape[-1] != action_dim:
                    if actions_tensor.shape[-1] < action_dim:
                        # 需要 padding
                        pad_size = action_dim - actions_tensor.shape[-1]
                        actions_tensor = torch.nn.functional.pad(actions_tensor, (0, pad_size))
                    else:
                        # 需要截断
                        actions_tensor = actions_tensor[..., :action_dim]
                
                # 归一化：y = 2 * (x - min) / (max - min) - 1
                # actions_tensor: (B, T, D), action_min: (D,), safe_denom: (D,)
                # PyTorch 会自动广播
                actions_norm = 2 * (actions_tensor - action_min) / safe_denom - 1
                # 对于 denom == 0 的维度，设置为 0
                actions_norm = torch.where(
                    mask.unsqueeze(0).unsqueeze(0),  # (1, 1, D)
                    actions_norm,
                    torch.zeros_like(actions_norm)
                )
                
                return actions_norm
        # 如果没有统计信息，返回原值（假设已经归一化）
        if isinstance(actions, np.ndarray):
            actions_tensor = torch.as_tensor(actions, dtype=torch.float32)
        else:
            actions_tensor = actions if isinstance(actions, torch.Tensor) else torch.as_tensor(actions, dtype=torch.float32)
    ...
    if use_inpainting:
        ...
        print(f"   - Source: {inpainting_source}")
        print(f"   - Length: {inpainting_length} timestep(s)")
        print(f"   - Guide strength: {guide_strength} ({'hard' if guide_strength >= 0.999 else 'soft'} inpainting)")
    else:
        print(f"🎨 INPAINTING MODE: Disabled")
    ...
    # Inpainting 相关变量
    last_normalized_chunk: torch.Tensor | None = None  # 保存上一次归一化后的预测 chunk（用于 inpainting）
    ...
    for data_step, batch in iterator:
        ...
        if should_infer:
            ...
            # 准备 inpainting 的 guided_action（如果启用）
            guided_action = None
            if use_inpainting and inpainting_source != "none":
                if inpainting_source == "previous_pred":
                    if last_normalized_chunk is not None:
                        # 使用上一次预测的 chunk 的**最后** N 个时间步（而不是前 N 个）
                        # 这样才能保证动作的连续性：上一次 chunk 的最后一个时间步是当前时间步之前最近的动作
                        T_g = min(inpainting_length, last_normalized_chunk.shape[1])
                        # 取最后 T_g 个时间步
                        guided_action = last_normalized_chunk[:, -T_g:, :].to(device)
                        if data_step > 0:
                            print(f"🎨 Using previous prediction (last {T_g} timesteps) for inpainting: shape {guided_action.shape}")
                    else:
                        # 首次推理时，last_normalized_chunk 为 None，跳过 inpainting
                        if data_step == 0:
                            print(f"🎨 Inpainting skipped at first step (no previous prediction available)")
                elif inpainting_source == "ground_truth":
                    # 使用 ground truth action 的前 N 个时间步
                    # 需要归一化 ground truth
                    gt_action_tensor = torch.as_tensor(gt_action, dtype=torch.float32).unsqueeze(0)  # (1, D)
                    # 扩展为 (1, T_g, D) 格式
                    T_g = min(inpainting_length, n_actions)
                    gt_action_expanded = gt_action_tensor.unsqueeze(1).repeat(1, T_g, 1)  # (1, T_g, D)
                    # 归一化
                    guided_action = normalize_actions(gt_action_expanded, action_dim, dataset_stats)
                    guided_action = guided_action.to(device)
                    if data_step >= 0:
                        print(f"🎨 Using ground truth for inpainting: shape {guided_action.shape}, T_g={T_g}")
            ...
            with torch.inference_mode():
                if use_inpainting and guided_action is not None:
                    pred_actions = policy.predict_action_chunk(
            ...
            # 保存归一化后的 chunk（用于下一次 inpainting）
            if use_inpainting and inpainting_source == "previous_pred":
                # pred_actions 已经是归一化的，直接保存
                # pred_actions shape: (batch_size, chunk_size, action_dim)
                last_normalized_chunk = pred_actions.detach().cpu()
                if data_step == 0 or (data_step % 10 == 0):  # 每 10 步打印一次，避免输出过多
                    print(f"🎨 Saved normalized chunk for next inpainting: shape {last_normalized_chunk.shape}, "
                          f"will use last {min(inpainting_length, last_normalized_chunk.shape[1])} timesteps")
        else:
            # 不需要推理：复用上一次的预测结果
                    ...
                    if use_inpainting and inpainting_source != "none":
                        if inpainting_source == "previous_pred":
                            if last_normalized_chunk is not None:
                                ...
                                # 首次推理时，last_normalized_chunk 为 None，跳过 inpainting
                                print(f"🎨 Inpainting skipped (no previous prediction available)")
                        elif inpainting_source == "ground_truth":
                            gt_action_tensor = torch.as_tensor(gt_action, dtype=torch.float32).unsqueeze(0)
                            T_g = min(inpainting_length, n_actions)
                            gt_action_expanded = gt_action_tensor.unsqueeze(1).repeat(1, T_g, 1)
                            guided_action = normalize_actions(gt_action_expanded, action_dim, dataset_stats)
                            guided_action = guided_action.to(device)
                            print(f"🎨 Using ground truth for inpainting: shape {guided_action.shape}, T_g={T_g}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-inpainting', action='store_true',
                       help='Enable inpainting (use guided_action prefix for action generation)')
    parser.add_argument('--inpainting-source', type=str, default='previous_pred',
                       choices=['previous_pred', 'ground_truth', 'none'],
                       help='Source for inpainting: previous_pred (use previous prediction), ground_truth (use GT), or none (disabled)')
    parser.add_argument('--inpainting-length', type=int, default=1,
                       help='Number of timesteps to use for inpainting prefix (default: 1)')
    parser.add_argument('--guide-strength', type=float, default=0.5,
                       help='Inpainting guide strength (0.0-1.0). >= 0.999 uses hard inpainting, < 0.999 uses soft inpainting (default: 0.5)')
    ...
    if args.task_description:
        ...
        print(f"Task Description: Will use task from dataset")
    print(f"Use Inpainting: {args.use_inpainting}")
    if args.use_inpainting:
        print(f"Inpainting Source: {args.inpainting_source}")
        print(f"Inpainting Length: {args.inpainting_length}")
    ...
    eval_on_dataset(
        ...
        inpainting_source=args.inpainting_source,
        inpainting_length=args.inpainting_length,
